      ...
      {(!mob || sidebarOpen) && (
        <motion.aside
          ...
          <div className="px-4 pt-5 pb-4 border-b border-border flex items-center justify-between">
            <div>
              <UpscaleLogo width={150} height={30} id="sb" />
              <div className="mt-2 flex items-center gap-2">
                <span className="text-[9px] font-semibold tracking-[0.2em] uppercase text-muted-foreground">Project Tracker</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-black" style={{ background: `rgba(147,197,253,.1)`, color: '#2563EB' }}>BETA</span>
              </div>
            </div>
            ...
            {nav.map((ni) => (
              <div key={ni.id}>
                ...
                <motion.button whileHover={{ x: 4 }} whileTap={{ scale: .97 }} onClick={() => goTo(ni.id)}
                  ...
                  {ni.label}
                  {ni.id === "todo" && <span className="ml-auto text-[9px] px-1.5 rounded-full bg-border text-muted-foreground">45</span>}
                  {ni.id === "tasks" && myTasks.filter(t => t.status !== "Done").length > 0 && <span className="ml-auto text-[9px] px-1.5 rounded-full" style={{ background: `rgba(147,197,253,.15)`, color: '#2563EB' }}>{myTasks.filter(t => t.status !== "Done").length}</span>}
                  {ni.id === "admin" && <span className="ml-auto text-[8px] px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(248,113,113,.12)', color: '#F87171' }}>Admin</span>}
                  {ni.id === "help" && <span className="ml-auto text-[8px] px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(96,165,250,.12)', color: '#60A5FA' }}>?</span>}
                </motion.button>
              </div>
            ))}
          </nav>
          ...
          <div className="px-3 pb-2">
            <button onClick={() => { setCmdOpen(true); setSidebarOpen(false); }} className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-xs bg-card border border-border text-muted-foreground cursor-pointer">
              ...
              <span>Search</span>
              <span className="ml-auto px-1.5 py-0.5 rounded text-[9px] font-mono bg-border text-muted-foreground">&#8984;K</span>
            </button>
          </div>
