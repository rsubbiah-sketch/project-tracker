...
export default function Comments({ eId, com, setCom, rep, setRep }: CommentsProps) {
  ...
  return(<Cd delay={.15} hover={false} className="p-4 md:p-5">
    ...
    <div className="space-y-3"><AnimatePresence>{vis.map((c,i)=>{const cr=rep.filter(r=>r.cId===c.id);return(
      <motion.div key={c.id} id={`comment-${c.id}`} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{delay:i*.04}} className="rounded-xl p-4 border border-border" style={{background:c.resolved?'hsla(var(--muted)/0.4)':'var(--muted)',opacity:c.resolved?.5:1}}>
        <div className="flex gap-3"><Av user={c.author} size={28}/><div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2"><span className="text-xs font-bold text-foreground">{c.author.name}</span><span className="px-1.5 py-0.5 rounded text-xs bg-border text-muted-foreground">{c.author.role}</span><span className="text-xs ml-auto text-muted-foreground">{fmtD(c.ts)}</span></div>
        ...
        {cr.length>0&&<div className={`${mob?"ml-4":"ml-10"} mt-3 space-y-3`}>{cr.map(r=><div key={r.id} id={`reply-${r.id}`} className="flex gap-2.5 pl-3 border-l-2 border-border"><Av user={r.author} size={mob?18:22}/><div className="flex-1"><div className="flex items-center gap-2 mb-0.5 flex-wrap"><span className="text-xs font-bold text-foreground">{r.author.name}</span><span className="text-xs text-muted-foreground">{fmtD(r.ts)}</span></div><p className="text-xs leading-relaxed text-secondary-foreground">{mention(r.body)}</p><button onClick={()=>like("r",r.id)} className="flex items-center gap-1 text-xs mt-1 bg-transparent border-none cursor-pointer" style={{color:r.likes.includes(ME.id)?u.err:'var(--muted-foreground)'}}><I name="heart" size={10} color={r.likes.includes(ME.id)?u.err:'var(--muted-foreground)'}/>{r.likes.length>0&&r.likes.length}</button></div></div>)}</div>}
