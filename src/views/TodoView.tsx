          ...
          <div className="space-y-3">{phase.items.map((task,ti)=>(
            <motion.div key={task.id} initial={{opacity:0,x:-10}} animate={{opacity:1,x:0}} transition={{delay:ti*.03}} className="px-3 py-2.5 rounded-lg bg-muted border border-border">
              <div className="flex items-center gap-2 md:gap-3">
                ...
                {!mob&&<span className="text-xs px-2 py-0.5 rounded-md flex-shrink-0 bg-card text-muted-foreground">{task.o}</span>}
              </div>
              <div className="flex gap-1 flex-wrap mt-3 ml-7 md:ml-8">{so.map(x=>{const sty=sC[x]||sC.Pending;return<motion.button key={x} whileTap={{scale:.88}} onClick={()=>setSt(phase.id,task.id,x)} className="px-2 py-0.5 rounded-md cursor-pointer text-xs" style={{border:`1px solid ${task.s===x?sty.c:'var(--border)'}`,background:task.s===x?sty.bg:"transparent",color:task.s===x?sty.c:'var(--muted-foreground)',fontWeight:task.s===x?700:400}}>{x}</motion.button>;})}</div>
            </motion.div>))}</div>
